...
*****************2024-09-29*****************
服务端：  
	服务端默认不开始FastReport服务端打印功能。  
	在源码 One.inc配置文件开启，默认关闭，One_NOFastReport改成One_FastReport即可  
	增加锁单功能接口  
客户端：  
	增加Demo-OneClientDemoBillLock  
	场景 比如多个人同时编辑一个订单,防止其它人打开或保存时。。就可以利用此功能。  
	只解决部份场景。提供一个思路。   
	
*****************2024-09-27*****************  
服务端：  
	解决FastApi一些BUG问题，个别人有用到。  
	解决打开数据或执行存储过程参数类型是ftTimeStamp传输问题,可能其它数据库还有其它问题，碰到联系。  
客户端：  
	增加Demo-OneClientDemoOpenIncCache  
	场景 比如我在登时取下商品信息缓存在本地，提供给其它界面做选择或数据查看。  
	但别人有更新或增加数据（删除除外),我如何以最小成本最小数据量更新本地数据缓存，那么就可以使用TOneDataSet.OpenDataIncCache  
	会以一个主键还有一个最大值键为基础，去服务端查询比最大值大的数据，然后取下来与本地数据进行合并，当然这不是最优方法,只是提供了    
	一定场景的解决方案。    
	
*****************2024-09-16*****************  
服务端：  
	解决高并发情况，不稳定情况，在Win下采用mormot2 http.sys  
		mormot2自代的一些HTTP服务，目前不够稳定  
	解决账套锁定失败，在并发多时  
	解决Controll层，单例模型，上下文httpContext ThreadID冲突问题。。HTTP底程线程会复用  
	解决一系列问题和优化  
客户端：  
	增加两个Demo   
	OneCleintDemoCustTranB 同个事务执行DML语句，并通过判断语句来处理相关 业务形态,可以N个DML以及判断语句在同个事务  
	OneCleintDemoCustTranC 同个事务提交Deltal数据集,通过判断语句来处理相关业务形态,只能保存一次,不能修改保存判断，在修改保存在判断。。在同个事务  
